...
void LinkedList<T>::insertOrdered(const T &newData)
{
  ...
  Node *currentAddress = this->getHeadPtr();
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    T currentData = currentAddress->data;
    ...
    else
    {
      ...
      {
        this->head_ = newNode;
        currentAddress->prev = newNode;
        newNode->prev = nullptr;
        newNode->next = currentAddress;
      }
      else
      {
        Node *prevAddress = currentAddress->prev;
        prevAddress->next = newNode;
        currentAddress->prev = newNode;
        newNode->prev = prevAddress;
        newNode->next = currentAddress;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T> &other) const
{
  ...
  // the function.
  LinkedList<T> merged;
  // i가 left / j가 right / k가 merged용 인덱스입니다.
  int i = 0;
  int j = 0;

  Node *leftPointer = left.getHeadPtr();
  Node *rightPointer = right.getHeadPtr();
  Node *mergedPointer = nullptr;

  while ((i < left.size_) && (j < right.size_))
  {
    if (leftPointer->data > rightPointer->data)
    {
      Node *newNode = new Node(rightPointer->data);

      if (mergedPointer == nullptr)
      {
        merged.head_ = newNode;
      }
      else
      {
        mergedPointer->next = newNode;
        newNode->prev = mergedPointer;
      }
      merged.tail_ = newNode;
    ...
    else
    {
      ...
      Node *newNode = new Node(leftPointer->data);
